...
namespace ceps{
	namespace interpreter{
						...
						node_t predecessor, Call_parameters* params)
		{
        	...
        	node_vec_t args{get_args(*params)};
            if(args.size() != 1)
                 throw semantic_exception{root_node,"as_uint8() requires one argument."};

			if ( is<Ast_node_kind::int_literal>(args[0]) )
              return mk_uint8(value(as_int_ref(args[0]))) ;
			if ( is<Ast_node_kind::long_literal>(args[0]) )
              return mk_uint8(value(as_int64_ref(args[0]))) ;
			if ( is<Ast_node_kind::float_literal>(args[0]) )
              return mk_uint8(value(as_double_ref(args[0])));
			if ( is<Ast_node_kind::uint8>(args[0]) )
              return mk_uint8(value(as_uint8_ref(args[0])));
		...
		node_t first_lines_of_string(node_t root_node, Symboltable & sym_table, Environment& env,node_t parent_node, node_t predecessor, Call_parameters* params)
		{
        	node_vec_t args{get_args(*params)};
			...
			for (int i = 0; i < num_of_comp;++i){
				...
				if (p == std::string::npos) { return(mk_string(s)); }
				last_match = p + 1;
		...
		)
{
	...
	if (!is_an_identifier(fcall_target)){
		...
		if (name(id) == "hd"){
		 ...
		 } else if (name(id) == "sqrt") {
			node_vec_t args{get_args(*static_cast<ceps::ast::Call_parameters*>(params_))};
			if (args.size() != 1)
				 throw semantic_exception{root_node,"sqrt: Expecting 1 argument"};
			auto arg = args[0];

			if (arg->kind() == Kind::float_literal)
				return new ceps::ast::Double(std::sqrt(value(as_double_ref(arg))),ceps::ast::all_zero_unit(),nullptr,nullptr,nullptr);
			else if (arg->kind() == Kind::int_literal)
				return new ceps::ast::Double(std::sqrt((double)value(as_int_ref(arg))), ceps::ast::all_zero_unit(), nullptr, nullptr, nullptr);
		 	else
			 {
				 if (ceps::ast::is<Ast_node_kind::symbol>(arg)) symbols_found = true;
				 auto fp = new ceps::ast::Call_parameters();
