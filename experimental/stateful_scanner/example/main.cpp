...
template<typename T> struct nonmutable_string
{
 ...
 nonmutable_string(T* p,size_type s):ptr_(p),size_(s){}
...
inline bool operator == (nonmutable_string<char> const &  lhs ,  nonmutable_string<char> const &  rhs)
{
...
bool operator == (nonmutable_string<char> const &  lhs ,  char * rhs)
{
...
template<typename T>struct Memory
{
  ...
  Memory(Memory && rhs) : ptr_(rhs.ptr_), size_(rhs.size_), pos_(rhs.pos_) {rhs.ptr_ = nullptr;}
  T * ptr_;
  ...
  bool get(T& ch)
   ...
   ~Memory () {if (ptr_ != nullptr) delete[] ptr_; }
};
...
public:
 ...
 struct Tokentable
 {
   ...
   void clear_and_read_table(char * tbl [], size_t size)
   {
    toks_.resize(size);
 ...
 struct Token
 {
  ...
  union
  {
   ...
   enum {
    ...
    TOK_MATCH_ANY = 17, TOK_CALL_LEXER, TOK_ELSE, TOK_EXIT, TOK_REWIND
...
/* int rw_rule_in_execution_;
 ...
 bool expr_parser_match(int kind,char* content = nullptr, bool check = true, bool b_call_gettoken = true, bool b_throw = true)
 {
  if (check)
...
int parse_sideeffect_ASSIGN()
 ...
 bool gettoken(Token& t,int lexer_id)
 {
  ...
  for(;;)
  {
   ...
   do
   {
    ...
    for(decltype(n) i = 0; i < n;++i)
    {
     ...
     } else if (opcode.t_.kind() == Token::TOK_ELSE)
     {
       ...
       else if (t.kind() == Token::TOK_ID && t.sval_ == opcode.t_.sval_){/*std::cout << "matched:" << t.sval_ <<" " << i << std::endl;*/match = true;}
   ...
   //std::cout << "******Selected rule: #" << min_matching_idx  << "\n";
...
int main(int argc, char *argv[])
{
  ...
  if (argc == 1)
    {std::cout << "Usage: example FILE\n";return 1;}

  Memory<char> mem;

  if(!readfile_to_memory(mem, argv[1] )) {std::cerr << "Cannot open/read '"<< argv[1] <<"'\n"; return 2;}


  Statefulscanner<Memory<char>,char> scanner(std::move(mem));
  size_t ttt = 0;
  for(;default_ops[ttt++];);
...
/*
   if (t.kind() == Statefulscanner<Memory<char>,char>::Token::TOK_ID)
     std::cout << "<" << t.sval_ << "> ";
   else if (t.kind() == Statefulscanner<Memory<char>,char>::Token::TOK_STR)
     std::cout << "\"" << t.sval_ << "\" ";
   else if (t.kind() == Statefulscanner<Memory<char>,char>::Token::TOK_INT64)
     std::cout << "<INT64=" << t.v.ival64_ << "> ";
   else if (t.kind() == Statefulscanner<Memory<char>,char>::Token::TOK_DOUBLE)
     std::cout << "<DOUBLE=" << t.v.dval_ << "> ";
   else if (t.kind() == Statefulscanner<Memory<char>,char>::Token::TOK_OP)
     std::cout << "<OP=" << t.v.ival32_ <<" '"<<t.sval_ <<  "' > ";
   else if (t.kind() == Statefulscanner<Memory<char>,char>::Token::TOK_TOK)
     std::cout << "<TOKEN=" << t.v.ival32_ <<" '"<<t.sval_ <<  "' > ";
   else if (t.kind() == Statefulscanner<Memory<char>,char>::Token::TOK_ENDL)
     std::cout << "<TOKEN=ENDL> ";
   else if (t.kind() == Statefulscanner<Memory<char>,char>::Token::TOK_UNKNOWN)
     std::cout << "<?> ";
   std::cout << std::endl;*/
